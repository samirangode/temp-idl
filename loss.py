...
class SoftmaxCrossEntropy(Criterion):
    ...
    def forward(self, x, y):
        ...
        # -np.log(Y[np.nonzero(D)]).item()
        # print(x.shape)
        #x = x.reshape(x.shape[1])
        self.softmax = np.divide(np.exp(self.logits),np.sum(np.exp(self.logits),axis=1, keepdims = True))
        #loss = -np.log(x[np.nonzero(y)]).item()
        loss = - np.sum(self.labels*np.log(self.softmax),axis = 1)
        return loss
        #raise NotImplemented
